...
export const email = builder("email", {
  id: varchar("id").$type<EmailId>().notNull().primaryKey(),
  from_user_id: varchar("from_user_id").$type<UserId>(),
  organization_id: varchar("organization_id").$type<OrganizationId>().notNull(),
  from_user_entity_id: varchar("from_user_entity_id").$type<UserEntityId>(),
  type: varchar("type").notNull(),
  message_id: varchar("message_id").$type<MessageId>().notNull(),
  in_reply_to_message_id: varchar("in_reply_to_message_id").$type<MessageId>(),
  ...
  webhook_message_id: varchar("webhook_message_id").$type<WebhookMessageId>(),
